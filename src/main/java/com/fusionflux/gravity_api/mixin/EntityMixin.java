...
public abstract class EntityMixin{
    ...
    @Shadow
    protected static Vec3d adjustMovementForCollisions(Vec3d movement, Box entityBoundingBox, List<VoxelShape> shapes) {
        return null;
    }

    @Shadow @Final protected Random random;

//    @Shadow public abstract Vec3d adjustMovementForCollisions(@Nullable Entity entity, Vec3d movement, Box entityBoundingBox, World world, List<VoxelShape> collisions);

    @Shadow public abstract World getWorld();

    @ModifyArg(method = "applyGravity", at = @At(value = "INVOKE", target = "Lnet/minecraft/util/math/Vec3d;add(DDD)Lnet/minecraft/util/math/Vec3d;"), index = 1)
...
//    }
    ...
    private void redirect_adjustMovementForCollisions_offset_1(Args args, @Local(ordinal = 1) Vec3d vec3d) {
        //ToDo: Cleanup?
//        Vec3d rotate = args.get(0);
        Vec3d rotate = vec3d;
        rotate = RotationUtil.vecPlayerToWorld(rotate, GravityChangerAPI.getGravityDirection((Entity)(Object)this));
//        args.set(0, rotate);
    ...
    //ToDo: Check (adjustSingleAxisMovementForCollisions -> adjustMovementForCollisions)
    @ModifyVariable(
...
//
    ...
    //ToDo: Check (INVOKE_ASSIGN -> STORE)
    @ModifyVariable(
...
//            method = "tryTickInVoid",
    ...
    private void inject_attemptTickInVoid(CallbackInfo ci) {
        //ToDo: Check
        if (GravityChangerMod.config.voidDamageAboveWorld && this.getY() > (double) (this.world.getTopY(Heightmap.Type.WORLD_SURFACE, (int)this.getX(), (int)this.getZ()) + 256)) {
            this.tickInVoid();
        }
    }
