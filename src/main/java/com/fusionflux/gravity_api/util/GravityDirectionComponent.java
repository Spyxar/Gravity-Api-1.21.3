...
import com.fusionflux.gravity_api.util.updatednetworking.packet.DefaultGravityPacket;
import com.fusionflux.gravity_api.util.updatednetworking.packet.DefaultGravityStrengthPacket;
import com.fusionflux.gravity_api.util.updatednetworking.packet.InvertGravityPacket;
import com.fusionflux.gravity_api.util.updatednetworking.packet.OverwriteGravityPacket;
...
public class GravityDirectionComponent implements GravityComponent {
    ...
    @Override
    public void writeToNbt(@NotNull NbtCompound nbt, RegistryWrapper.WrapperLookup registries) {
        int index = 0;
...
//            GravityChannel.sendFullStatePacket(entity, NetworkUtil.PacketMode.EVERYONE, rotationParameters, true);
            ...
            NetworkUtil.getGravityComponent(entity).ifPresent(gc -> {
                ServerPlayNetworking.send((ServerPlayerEntity) entity, new OverwriteGravityPacket(entity.getId(), gc.getGravity(), true));
                ServerPlayNetworking.send((ServerPlayerEntity) entity, new DefaultGravityPacket(entity.getId(),gc.getDefaultGravityDirection(), rotationParameters, true));
                ServerPlayNetworking.send((ServerPlayerEntity) entity, new InvertGravityPacket(entity.getId(), gc.getInvertGravity(), rotationParameters, true));
                ServerPlayNetworking.send((ServerPlayerEntity) entity, new DefaultGravityStrengthPacket(entity.getId(), gc.getGravityStrength()));
        });
        }
    }
}
